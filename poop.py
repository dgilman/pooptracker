...
def track_poop():
    ...
    if not ((city_conf["lat_lt"] <= lat) and (city_conf["lat_gt"] >= lat)) or not (
        (city_conf["lon_lt"] <= lon) and (city_conf["lon_gt"] >= lon)
    ):
        return ""
...
""",
    ...
    if len(terminal_id) == 0:
        ...
        g.c.execute(
            "SELECT tail_wgs84 FROM omaha_sewers WHERE objectid = %s", (terminal_id,)
        )
    ...
    return json.dumps(
        {
            "sewers": u"[" + u",".join((x[0] for x in sewers)) + u"]",
            "terminal": terminal,
        }
    )
...
def get_terminals():
    def fn():
        query = "SELECT tail_wgs84 FROM {0}_sewers WHERE downstream IS NULL".format(
            request.args["city"]
        )
        g.c.execute(query)
...
def get_objectid():
    def fn(objectid):
        query = "SELECT sewer_wgs84 AS geojson FROM {0}_sewers WHERE objectid = %s".format(
            request.args["city"]
        )
